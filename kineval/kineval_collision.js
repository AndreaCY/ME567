...
function robot_collision_forward_kinematics(q)
{
    ...
    var R = matrix_multiply(matrix_multiply(generate_rotation_matrix_Z(q[5]),
    ...
    if (robot.links_geom_imported) {
        var offset_xform = matrix_multiply(generate_rotation_matrix_Y(-Math.PI/2),
...
function traverse_collision_forward_kinematics_joint(joint,mstack,q){
    ...
    var Qr;
    var q_ang = q[q_names[joint.name]];
    if (robot.links_geom_imported){
        if (joint.type === "revolute" ||  joint.type === "continuous"){
            Qr = quaternion_to_rotation_matrix(quaternion_normalize(quaternion_from_axisangle(q_ang,joint.axis)));
        }
        else if (joint.type === "prismatic"){
            Qr = generate_translation_matrix(q_ang*joint.axis[0],q_ang*joint.axis[1],q_ang*joint.axis[2]);
        }
        else{ Qr = generate_identity(4);}
    }
    else{ Qr = quaternion_to_rotation_matrix(quaternion_normalize(quaternion_from_axisangle(q_ang,joint.axis)));}
    mstack = matrix_multiply(matrix_multiply(mstack,matrix_multiply(D,R)), Qr);
